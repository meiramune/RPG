...
public class Main {
	public static void main(String[] args) {
		...
		Fighter figther = new Fighter("金太郎", "斧");
		...
		while (true) {
			...
			Human choiceman = choiceHuman(humans);

			// モンスターグループから1人選択
			Monster choicemon = choiceMonster(monsters);

			// 選ばれた人間が、選ばれたモンスターを攻撃
			choiceman.attack(choicemon);

			// モンスターのHPが0以下になれば、そのモンスターをモンスターリストから削除
			if (choicemon.getHp() <= 0) {
				System.out.println("★ 「" + choicemon.getName() + "」は倒れた。");
				monsters.remove(choicemon);

			}

			// モンスターリストに誰もいなくなれば、人間グループの勝利
			if (monsters.size() == 0) {
				System.out.println("\n★★ ==== 決着がついた！！ ==== ★★\n");
				System.out.println("\n#### 人間達は勝利した！！ ####\n");
				break;
			...
			// 人間グループから1人選択
			choiceman = choiceHuman(humans);
			// モンスターグループから1人選択
			choicemon = choiceMonster(monsters);
			// 選ばれたモンスターが、選ばれた人間を攻撃
			choicemon.attack(choiceman);
			// 人間のHPが0以下になれば、人間は倒れ、その人間をモンスターグループから削除
			if (choiceman.getHp() <= 0) {
				System.out.println("★ 「" + choiceman.getName() + "」は倒れた。");
				humans.remove(choiceman);
			}
